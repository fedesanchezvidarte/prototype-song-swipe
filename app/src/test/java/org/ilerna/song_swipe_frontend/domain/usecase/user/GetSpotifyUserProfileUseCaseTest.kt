...
class GetSpotifyUserProfileUseCaseTest {
    ...
    fun `invoke should return success with user when repository succeeds`() = runTest {
        ...
        coEvery { mockSpotifyRepository.getCurrentUserProfile() } returns NetworkResult.Success(expectedUser)
    ...
    fun `invoke should return error when repository fails`() = runTest {
        ...
        val errorCode = 500
        coEvery { mockSpotifyRepository.getCurrentUserProfile() } returns NetworkResult.Error(errorMessage, errorCode)
    ...
    fun `invoke should return error with 401 when unauthorized`() = runTest {
        // Given
        coEvery { mockSpotifyRepository.getCurrentUserProfile() } returns NetworkResult.Error("Unauthorized", 401)
    ...
    fun `invoke should return error without code when code is null`() = runTest {
        // Given
        coEvery { mockSpotifyRepository.getCurrentUserProfile() } returns NetworkResult.Error("Unknown error", null)
    ...
    fun `invoke should call repository exactly once`() = runTest {
        // Given
        coEvery { mockSpotifyRepository.getCurrentUserProfile() } returns NetworkResult.Error("Error", null)
